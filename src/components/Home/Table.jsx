...
export function Table() {
    ...
    return (
        <div className='w-4/5 h-3/4 mx-auto my-4'>
            <div className='grid grid-cols-3'>
                {TABLE_HEAD.map((head) => (
                    <div key={head} className='bg-cyan-700 py-4'>
                        <p className='-ml-4 font-bold text-white text-xl text-center'>{head}</p>
                    </div>
                ))}
            </div>
            <div className='scrollbar h-full overflow-y-auto' id='scrollableDiv'>
                <InfiniteScroll
                    ...
                    {cities.map((item) => (
                        <div key={item.geoname_id} className='grid grid-cols-3 py-1'>
                            <p
                                className='text-xl bg-white p-2 m-1 text-center cursor-pointer hover:bg-sky-700 hover:text-white'
                                onClick={() => {
                                    navigate("/weather", {
                            ...
                            <p className='text-xl bg-white p-2 m-1 text-center'>{item.cou_name_en}</p>
                            <p className='text-xl bg-white p-2 m-1 text-center'>{item.timezone}</p>
                        </div>
                    ))}
                </InfiniteScroll>
            </div>
